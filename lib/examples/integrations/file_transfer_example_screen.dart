...
/// 演示通过可复用的 [FileTransferService] 进行多部分文件上传和带进度的下载，
/// 针对两个知名的公共测试端点（无需认证）：
/// - 上传：`httpbin.org/post`，回显接收到的内容。
/// - 下载：`picsum.photos`，一个随机图片服务（也在此模板的展示页面中使用）。
///
/// 将 `core/network/integrations/file_transfer_service.dart` 复制到真实的功能模块中
/// （头像上传、文档下载、媒体同步...）并指向你自己的后端。
///
/// 仅非 Web 平台：保存到临时文件需要真实的文件系统，而浏览器无法像其他平台那样暴露文件系统。
